...
const PBKDF2_ITERS = 100000; // Cloudflare-safe upper bound
...
export async function onRequest(context) {
  ...
  try {
        ...
        return json(
          {
            ok: false,
            error:
              "DB not reachable. Check D1 binding name = DB and that it is bound in the correct environment.",
    ...
    // AUTH ROUTES
    // -----------------------------
    if (request.method === "POST" && endpoint === "register") {
      return await register(request, env);
    }

    if (request.method === "POST" && endpoint === "login") {
    ...
    if (endpoint === "orders" && request.method === "GET") {
...
async function register(request, env) {
  ...
  if (!email || password.length < 8) {
    return json(
      { error: "Email required and password must be 8+ chars." },
      400
    );
  }

  if (!env?.DB) {
    return json({ error: "DB binding missing (env.DB)" }, 500);
  }
  ...
  await env.DB
    .prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `)
    .bind(id, email, phone || null, name || null, salt, hash, createdAt)
    .run();
...
async function googleStart(request, env) {
  ...
  if (!base || !env?.GOOGLE_CLIENT_ID || !env?.GOOGLE_CLIENT_SECRET) {
    return json(
      {
        error:
          "Google OAuth not configured. Need SITE_BASE_URL, GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET.",
      },
      500
    );
  }
...
async function googleCallback(request, env) {
  ...
  // Since your schema has password_salt/password_hash NOT NULL,
  // create placeholder values for Google-only accounts
  if (!userId) {

...
// IMPORTANT: Cloudflare WebCrypto PBKDF2 iterations limit is 100000
...
export async function onRequest(context) {
  ...
  // params.path is an array for [[path]].js
  ...
  try {
    // Basic same-origin protection for browser state-changing requests
    ...
    // Always JSON (so you don’t get HTML error pages)
...
function redirectWithCookies(location, cookieList = []) {
  const headers = new Headers({ Location: location });
  for (const c of cookieList) headers.append("Set-Cookie", c);
...
async function googleCallback(request, env) {
  ...
  if (!userId) {
    ...
    // If user exists from email/password but has no name, set it
    await env.DB.prepare("UPDATE users SET name = ? WHERE id = ?")
      .bind(givenName, userId)
      .run();
  }

  // Create session (same as email/password flow)
